...
const editor = useEditor({
  extensions: [StarterKit, Underline, Link.configure({ openOnClick: false }), Strike],
...
<template>
  <div class="mx-40 overflow-hidden rounded-md border bg-white dark:bg-neutral-900">
    ...
    <div
      v-if="editor"
      class="flex flex-wrap gap-2 border-b bg-gray-50 p-2 dark:bg-gray-950"
    >
      <!-- Text Styles -->
      <button
        type="button"
      ...
      <button
        ...
        :class="{ 'bg-gray-300 dark:bg-gray-600': editor.isActive('') }"
      >
        Underline
      </button>
    ...
    <!-- Editor -->
     <EditorContent
      v-if="editor"
      :editor="editor"
      class="prose dark:prose-invert min-h-[150px] max-w-full p-4 [&_.ProseMirror]:focus:outline-none"
    />
  </div>
</template>
